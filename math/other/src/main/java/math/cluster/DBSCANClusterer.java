...
public class DBSCANClusterer<T extends Clusterable> extends Clusterer<T> {
    ...
    public DBSCANClusterer(final double eps, final int minPts, final DistanceMeasure measure)
        ...
        if (eps < 0.0d) {
            throw new Exception("eps should greater then 0");
        }
        if (minPts < 0) {
            throw new Exception("eps should greater then 0");
        }
     ...
     */
    @Override
    public List<Cluster<T>> cluster(final Collection<T> points) throws Exception {


        if(points == null){
        	throw new Exception("data set should not be null");
        }
