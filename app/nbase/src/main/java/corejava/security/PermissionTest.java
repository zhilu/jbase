...
public class PermissionTest {
	...
	public static void main(String[] args) {
		String policy = new PermissionTest().getClass().getResource("PermissionTest.policy").getPath();
		System.setProperty("java.security.policy", policy.substring(1, policy.length()));
...
/**
 * This frame contains a text field for inserting words into a text area that is
...
class PermissionTestFrame extends JFrame {
	...
	/**
	 * Tries to insert words into the text area. Displays a dialog if the
	 * attempt fails.
	 * 
	 * @param words
	 *            the words to insert
...
/**
 * A text area whose append method makes a security check to see that no bad
 * words are added.
 */
@SuppressWarnings("serial")
