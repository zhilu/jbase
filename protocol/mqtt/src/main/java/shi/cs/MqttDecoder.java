...
import shi.proto.MqttConnectReturnCode;
import shi.proto.MqttMessage;
import shi.proto.MqttMessageFactory;
import shi.proto.MqttMessageType;
import shi.proto.MqttQoS;
import shi.proto.MqttTopicSubscription;
import shi.proto.MqttVersion;
import shi.proto.exception.MqttIdentifierRejectedException;
...
import shi.cs.MqttDecoder.DecoderState;

import static shi.util.MqttCodecUtil.isValidClientId;
import static shi.util.MqttCodecUtil.isValidMessageId;
import static shi.util.MqttCodecUtil.isValidPublishTopicName;
import static shi.util.MqttCodecUtil.resetUnusedFields;
import static shi.util.MqttCodecUtil.validateFixedHeader;

import java.util.ArrayList;
import java.util.List;

/**
 * Decodes Mqtt messages from bytes, following <a href=
...
public final class MqttDecoder extends ReplayingDecoder<DecoderState> {
	...
	/**
	 * States of the decoder. We start at READ_FIXED_HEADER, followed by
	...
	protected void decode(ChannelHandlerContext ctx, ByteBuf buffer, List<Object> out) throws Exception {
		...
		case READ_PAYLOAD:
			try {
				final Result<?> decodedPayload = decodePayload(buffer, mqttFixedHeader.messageType(),
						bytesRemainingInVariablePart, variableHeader);
				payload = decodedPayload.value;
				bytesRemainingInVariablePart -= decodedPayload.numberOfBytesConsumed;
				if (bytesRemainingInVariablePart != 0) {
					throw new DecoderException("non-zero remaining payload bytes: " + bytesRemainingInVariablePart
	...
	private static MqttFixedHeader decodeFixedHeader(ByteBuf buffer) {
		...
		MqttFixedHeader decodedFixedHeader = new MqttFixedHeader(messageType, dupFlag, MqttQoS.valueOf(qosLevel),
				retain, remainingLength);
	...
	private static Result<MqttConnectVariableHeader> decodeConnectionVariableHeader(ByteBuf buffer) {
		...
		if (mqttVersion == MqttVersion.MQTT_3_1_1) {
			...
			if (!zeroReservedFlag) {
				// MQTT v3.1.1: The Server MUST validate that the reserved flag
				// in the CONNECT Control Packet is
				// set to zero and disconnect the Client if it is not zero.
				// See
				// http://docs.oasis-open.org/mqtt/mqtt/v3.1.1/os/mqtt-v3.1.1-os.html#_Toc385349230
				throw new DecoderException("non-zero reserved flag");
			}
		}

		final MqttConnectVariableHeader mqttConnectVariableHeader = new MqttConnectVariableHeader(
				mqttVersion.protocolName(), mqttVersion.protocolLevel(), hasUserName, hasPassword, willRetain, willQos,
				willFlag, cleanSession, keepAlive.value);
	...
	private static Result<MqttConnAckVariableHeader> decodeConnAckVariableHeader(ByteBuf buffer) {
		...
		final MqttConnAckVariableHeader mqttConnAckVariableHeader = new MqttConnAckVariableHeader(
				MqttConnectReturnCode.valueOf(returnCode), sessionPresent);
	...
	private static Result<MqttMessageIdVariableHeader> decodeMessageIdVariableHeader(ByteBuf buffer) {
		...
		return new Result<MqttMessageIdVariableHeader>(MqttMessageIdVariableHeader.from(messageId.value),
				messageId.numberOfBytesConsumed);
	}

	private static Result<MqttPublishVariableHeader> decodePublishVariableHeader(ByteBuf buffer,
		...
		final MqttPublishVariableHeader mqttPublishVariableHeader = new MqttPublishVariableHeader(decodedTopic.value,
				messageId);
	 ...
	 *            the buffer to decode from
	 * @param messageType
	 *            type of the message being decoded
	 * @param bytesRemainingInVariablePart
	 *            bytes remaining
	 * @param variableHeader
	 *            variable header of the same message
	 * @return the payload
	 */
	private static Result<?> decodePayload(ByteBuf buffer, MqttMessageType messageType,
	...
	private static Result<MqttConnectPayload> decodeConnectionPayload(ByteBuf buffer,
	...
	private static Result<MqttSubscribePayload> decodeSubscribePayload(ByteBuf buffer,
	...
	private static Result<MqttUnsubscribePayload> decodeUnsubscribePayload(ByteBuf buffer,
			int bytesRemainingInVariablePart) {
